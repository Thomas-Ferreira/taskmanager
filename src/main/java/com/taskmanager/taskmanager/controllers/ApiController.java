...
public class ApiController {
    ...
    public ResponseEntity<TaskResponse> updateTask(@PathVariable Long userId, @PathVariable Long taskId, @RequestBody TaskRequest taskRequest){

        Optional<Task> updatedTaskOptional = TaskService.updateTaskByIdAndUserId(taskId, userId, taskRequest);
        
        if (updatedTaskOptional.isPresent()) {
            TaskResponse taskResponse = TaskResponse.fromTask(updatedTaskOptional.get());
            return new ResponseEntity<>(taskResponse, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
